...
pub async fn fetch_data() -> Result<Vec<LightningResponse>, String> {
    let url = env::var("LIGHTNING_URL").expect("LIGHTNING_URL env var is not present");
    let response = HTTP_CLIENT.request(Method::GET, &url).send().await;
    ...
    match response.json::<Vec<LightningResponse>>().await {
        Ok(result) => Ok(result),
        Err(_) => Err(format!(
            "The response from {} is not valid as expected to defined schema.",
            url
        )),
    }
}
